...
def preprocess_rcc_visium(cfg: dict, debug: bool = False) -> None:
    ...
    # Compute spatial features per sample (KNN must stay within a single slide)
    score_cols = [c for c in adata.obs.columns if c.startswith('score_')]
    obs_parts = []
    for sid in adata.obs['sample_id'].unique():
        obs_mask = adata.obs['sample_id'] == sid
        ipos = np.where(obs_mask.values)[0]
        mini = ad.AnnData(
            obs=adata.obs[obs_mask].copy(),
            obsm={'spatial': adata.obsm['spatial'][ipos]},
        )
        compute_spatial_features(mini, score_cols=score_cols)
        obs_parts.append(mini.obs)
        del mini
    obs_with_feats = pd.concat(obs_parts).reindex(adata.obs.index)
    new_cols = [c for c in obs_with_feats.columns if c not in adata.obs.columns]
    for col in new_cols:
        adata.obs[col] = obs_with_feats[col].values
    del obs_parts, obs_with_feats
    gc.collect()
    print(f'  Spatial features added: {len(new_cols)} columns '
          f'({len(score_cols)} scores × 3 stats)')
